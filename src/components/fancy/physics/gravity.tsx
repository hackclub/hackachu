...
export const MatterBody = ({
  ...
  useEffect(() => {
    ...
    // Register once per physics world: the context identity only changes
    // when the world resets (e.g. on resize). Depending on children/props
    // objects here would despawn and respawn the body at its start position
    // on every parent re-render.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [context, x, y, angle, bodyType])
...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
    ...
    const initializeRenderer = useCallback(() => {
      ...
      // Listen for mouse events on the parent element instead of the canvas,
      // so bodies stay draggable even when the overlay itself has
      // pointer-events: none (clicks pass through to the page content).
      const mouseElement = canvas.current.parentElement ?? render.current.canvas
      ...
      if (grabCursor) {
        Events.on(engine.current, "beforeUpdate", (event) => {
          if (canvas.current) {
            const dragging = mouseConstraint.current?.body != null
            const over = touchingMouse()

            // Only intercept the pointer while hovering or dragging a body,
            // so the rest of the page stays clickable underneath the overlay.
            canvas.current.style.pointerEvents =
              over || dragging ? "auto" : "none"

            if (!mouseDown.current && !over) {
    ...
    // canvasSize is intentionally a dependency: handleResize clears the
    // physics world, so the new context identity prompts MatterBody children
    // to re-register their bodies into the fresh world.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const contextValue = useMemo(
      () => ({ registerElement, unregisterElement }),
      [registerElement, unregisterElement, canvasSize]
    )
